...
Message::Message(std::vector<packet>& packets) {
    ...
    if(total_size < 40) {
        throw CorruptMessage(std::string("This is a corrupt message. Only ") + std::to_string(total_size) + std::string(" bytes were received "));
        return;
    }
    ...
    } catch(const PacketDataNullPointerException& except) {
        delete [] marshalled_message;
        throw CorruptMessage(except.what());
    }
