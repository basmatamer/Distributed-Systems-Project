...
//    std::cout << "Number of windows is " << windows_num << std::endl;
    for(unsigned int i = 0; i < windows_num; i++) {
        bool window_result;
        try {
            window_result = sendWindow(flat_packets, flat_packets_sizes, receiver_address, i * WINDOW_SIZE, (i+1) * WINDOW_SIZE, ack_timeout_usec, resend_num);
        } catch(const SendingFailedException& except) {
            std::cout << "UDPEnhancedSocket::sendMessage: " << except.what() << std::endl;
    ...
    if(remainder_packets > 0) {
        ...
        bool remainder_result;
        try {
            remainder_result = sendWindow(flat_packets, flat_packets_sizes, receiver_address, windows_num * WINDOW_SIZE, flat_packets.size(), ack_timeout_usec, resend_num);
        } catch(const SendingFailedException& except) {
            std::cout << "UDPEnhancedSocket::sendMessage: " << except.what() << std::endl;
    ...
    while(true) {
        ...
        if(shouldSendAck(packet_insert_result, packet_unique_id)) {
            ...
            try {
                ...
                std::cout << "UDPEnhancedSocket::receiveMessage " << except.what() << std::endl;
                throw except;
            }
        ...
        switch(packet_insert_result) {
            ...
            case message_completed:
                ...
                } catch(const CorruptMessage& except) {
                    new_message = NULL;
                    std::cout << except.what() << std::endl;
...
//    std::cout << " for window " << ack_on->getPacketNo() / WINDOW_SIZE << std::endl;
    ...
    if(len_sent == -1) {
       throw SendingFailedException("Unable to send acknowledgment packet");
    }
    return len_sent;
}
    ...
    do {
       // trials++;
        ...
        for(unsigned int k = start_from; k < end_at_but_dont_send && !ack_result; k++) {
            ...
            if(len_sent < 1) {
                throw SendingFailedException("Unable to send packet " +  std::to_string(k) + " in sendWindow");
            }
