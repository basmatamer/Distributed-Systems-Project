...
UDPSocket::UDPSocket():enabled(true) {
    ...
    if ((sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1) {
        ErrorHandler::die("Socket creation failed");
    }
    ...
    if (pthread_mutex_init(&mutex, NULL) != 0) {
        ErrorHandler::die("Mutex init failed");
    }
}

UDPSocket::UDPSocket(const UDPSocket& other):sock(other.sock), length_of_received(other.length_of_received), myAddr(other.myAddr), peerAddr(other.peerAddr), enabled(other.enabled), mutex(other.mutex) {
    ErrorHandler::displayWarning("Attempting to copy a UDP Socket");
}
UDPSocket::UDPSocket(UDPSocket&& other):sock(other.sock), length_of_received(other.length_of_received), myAddr(other.myAddr), peerAddr(other.peerAddr), enabled(other.enabled), mutex(other.mutex) {
...
UDPSocket::UDPSocket(uint32_t _myAddr, uint16_t _myPort):enabled(true) {
    ...
    if(!_initializeToReceive(_myAddr, _myPort)) {
        ErrorHandler::die("Initializing socket to receive failed");
    }
}
...
bool UDPSocket::_initializeToReceive(uint32_t _myAddress, uint16_t _myPort) {
    ...
    if(bind(sock, (sockaddr*) &myAddr, sizeof(myAddr)) == -1) {
        ErrorHandler::die("Binding my socket failed with error " + std::to_string(errno));
        return false;
    }
...
int UDPSocket::_send(const char * message, size_t message_length, uint32_t destination_ipAddress, uint16_t destination_portNumber) {
    ...
    if (len == -1) {
       ...
       // std::cout << len << " bytes were sent" << std::endl;
        ...
        unsigned int size_of_myAddr = sizeof(myAddr);
        getsockname(sock, (struct sockaddr *) &myAddr, &size_of_myAddr);
